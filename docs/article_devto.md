...
(Originally published on my blog: [https://roby73.hashnode.dev/microsoft-agent-framework-from-zero-to-multi-agent-pipeline](https://roby73.hashnode.dev/microsoft-agent-framework-from-zero-to-multi-agent-pipeline))

I have some background with other agent frameworks like CrewAI and LangGraph, so when Microsoft released the [Agent Framework](https://github.com/microsoft/agent-framework), a lightweight Python package for building AI agents with native MCP (Model Context Protocol) support, I was curious to give it a try. I decided to build something practical: a pipeline that reads a product backlog from a Markdown file and automatically creates Epics and Stories on Jira.
